...
module battle_pass::battle_pass_test {
  ...
  fun mint_default_and_transfer(admin: address, user: address, scenario: &mut Scenario){
    let mint_cap = test_scenario::take_from_address<MintCap>(scenario, admin);
    battle_pass::mint_default_and_transfer(&mint_cap, b"Battle Pass", b"Play Bushi to earn in-game assets by using this battle pass", b"dummy.com", user, test_scenario::ctx(scenario));
    test_scenario::return_to_address(admin, mint_cap);
  }

  fun create_upgrade_ticket_and_transfer(admin: address, user: address, xp_added: u64, scenario: &mut Scenario){
    let mint_cap = test_scenario::take_from_address<MintCap>(scenario, admin);
    let battle_pass = test_scenario::take_from_address<BattlePass>(scenario, user);
    let battle_pass_id = battle_pass::id(&battle_pass);
    battle_pass::create_upgrade_ticket_and_transfer(&mint_cap, battle_pass_id, xp_added, user, test_scenario::ctx(scenario));
    test_scenario::return_to_address(admin, mint_cap);
    test_scenario::return_to_address(user, battle_pass);
  }
