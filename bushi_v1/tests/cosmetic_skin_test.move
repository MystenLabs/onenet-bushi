...
module bushi::cosmetic_skin_test {
  ...
  use bushi::cosmetic_skin::{Self, CosmeticSkin, AllowUpdatesTicket, EWrongTicket, ECannotUpdate, ELevelGreaterOrEqualThanLevelCap};
  ...
  fun test_update(){
    ...
    // 1. make sure that the allow updates ticket is burned
  ...
  fun test_update_when_reached_level_cap() {
    ...
    // next transaction by admin to issue an allow updates ticket for the cosmetic skin of user
    test_scenario::next_tx(scenario, ADMIN);
    let allow_updates_ticket = create_allow_updates_ticket(cosmetic_skin_id, scenario);
    // admin transfers allow updates ticket to user
    transfer::public_transfer(allow_updates_ticket, USER);
